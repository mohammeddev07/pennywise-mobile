...
| Transactions             | `src/app/(tabs)/transactions.tsx`         | `/(tabs)/transactions`               | implemented                               | Full-ledger explorer over the server search/analyze contracts (`features/transactions/`): quick filters (date, type, categories, amount, payment, description), Advanced AND/OR builder (every field), 3-key sort, incremental paging, server-side totals for the whole match, and export of the exact applied filter/sort. TanStack Query owns rows/details/aggregates; Zustand owns drafts, applied filters and sorts per account+book. No persisted transaction array. |
